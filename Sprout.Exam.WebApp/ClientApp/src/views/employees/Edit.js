...
export class EmployeeEdit extends Component {
  ...
  constructor(props) {
    super(props);
      this.state = { id: 0, fullName: '', birthdate: '', tin: '', typeId: 1, typeName: '', isDeleted: 0, employeeTypes: [], loading: true,loadingSave:false };
  }
  ...
  render() {
    ...
    ? <p><em>Loading...</em></p>
    : EmployeeEdit.renderEmployeeTypes(this.state.employeeTypes, this);
  ...
  async getEmployee(id) {
    ...
    const data = await response.json();
      this.setState({ id: data.id, fullName: data.fullName, birthdate: data.birthdate, tin: data.tin, typeId: data.typeId, typeName: data.typeName, isDeleted: data.isDeleted, loading: false,loadingSave: false });
    }
